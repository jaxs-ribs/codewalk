...
PROJ_XCODEPROJ="$APP_DIR/WalkingComputer.xcodeproj"
DERIVED="$APP_DIR/build/DerivedData"
SCHEME="WalkingComputer"
APP_NAME="WalkingComputer"
BUNDLE_ID="com.example.walkingcomputer"
CONFIG="Debug"
LOG_DIR="$APP_DIR/logs"
LOG_FILE="$LOG_DIR/WalkingComputer_$(date +%Y%m%d_%H%M%S).log"
...
if [[ -f "$INFO_PLIST" ]]; then
  if ! /usr/libexec/PlistBuddy -c "Print :NSMicrophoneUsageDescription" "$INFO_PLIST" >/dev/null 2>&1; then
    ...
    /usr/libexec/PlistBuddy -c "Add :NSMicrophoneUsageDescription string 'Walking Computer needs microphone access for voice recording.'" "$INFO_PLIST" || true
  fi
fi
