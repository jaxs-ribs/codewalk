...
final class RelayWebSocket: NSObject, ObservableObject {
  ...
  var onStatus: ((String, String)->Void)?  // level, text
  ...
  func connect(url: String, sid: String, tok: String) {
    ...
    if state != .connecting {
      state = .connecting
      onStateChange?(.connecting)  // Notify callback
    }
  ...
  private func receiveLoop() {
    task?.receive { [weak self] result in
      ...
      case .failure(let err):
        self.state = .error
        self.onStateChange?(.error)  // Notify callback
  ...
  private func handleMessage(_ s: String) {
    ...
    print("[RelayWebSocket] Raw message received: \(s.prefix(200))...")
    ...
    if let t = obj["type"] as? String, t == "frame", let frame = obj["frame"] as? String, obj["b64"] as? Bool == false {
      print("[RelayWebSocket] Processing frame message")
      ...
      print("[RelayWebSocket] Processing direct message with type: \(obj["type"] ?? "unknown")")
  ...
  private func processInnerMessage(_ msg: [String: Any]) {
    if msg["type"] as? String == "ack" {
      ...
      lastAck = ackText
      onAck?(ackText)
    } else if msg["type"] as? String == "status" {
      let level = (msg["level"] as? String) ?? "info"
      let text = (msg["text"] as? String) ?? ""
      print("DEBUG: Received status - level: \(level), text: \(text)")
      onStatus?(level, text)
...
extension RelayWebSocket: URLSessionWebSocketDelegate {
  func urlSession(_ session: URLSession, webSocketTask: URLSessionWebSocketTask, didOpenWithProtocol protocol: String?) {
    state = .open
    onStateChange?(.open)  // Notify callback
    lastEvent = "ws:open"
    isConnecting = false
  ...
  func urlSession(_ session: URLSession, webSocketTask: URLSessionWebSocketTask, didCloseWith closeCode: URLSessionWebSocketTask.CloseCode, reason: Data?) {
    state = .closed
    onStateChange?(.closed)  // Notify callback
