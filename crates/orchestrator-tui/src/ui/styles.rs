...
impl Styles {
    ...
    pub fn for_output_line(line: &str) -> Style {
        // Check if it's a continuation line (starts with spaces)
        if line.starts_with("  ") && !line.starts_with("   ") {
            // It's a continuation, try to detect the original type from context
            // For now, use a slightly dimmed default style
            Style::default().fg(Color::Gray)
        } else if line.starts_with(prefixes::ASR) {
        ...
        } else if line.starts_with("Claude:") || line.starts_with("Devin:") {
    ...
    pub fn error_details() -> Style {
        Style::default().fg(Color::Gray)
    }
    
    pub fn error_key() -> Style {
        Style::default()
            .fg(Color::Green)
            .add_modifier(Modifier::BOLD)
    }
