...
impl Orchestrator {
    ...
    pub fn add_to_history(&mut self, user_msg: &str, assistant_msg: &str) {
        self.conversation_history
            .push(format!("User: {}", user_msg));
        self.conversation_history
            .push(format!("Assistant: {}", assistant_msg));
            ...
            Action::Edit {
                ...
                let updated = match patch_type {
                    PatchType::UnifiedDiff => {
                        ...
                        diffy::apply(&current, &patch_obj).context("Failed to apply diff")?
                    }
                    PatchType::YamlMerge => {
    ...
    pub fn handle_intent(&mut self, intent: Intent) -> Result<String> {
        match intent {
            ...
            Intent::Info { message } => Ok(message),
        }
    }
...
impl ContentGenerator {
    fn new() -> Result<Self> {
        ...
        let model = std::env::var("GROQ_GENERATOR_MODEL").unwrap_or_else(|_| {
            std::env::var("GROQ_LLM_MODEL")
    ...
    fn generate_content(&self, prompt: &str) -> Result<String> {
        ...
        if !status.is_success() {
            ...
            eprintln!(
                "Generator LLM error: Status={}, Body={}",
                status, error_text
            );
